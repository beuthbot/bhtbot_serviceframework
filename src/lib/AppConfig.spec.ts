...
test('use config', (t) => {
  testPorts.forEach(port=>{
    ...
    let service = new Service(testServiceName, config);
    ...
    t.assert(service.config.errorHandler === DefaultMiddleware.defaultErrorHandler);
    ...
    t.assert(service.config.middleware[0] === DefaultMiddleware.cookieMiddleware);
